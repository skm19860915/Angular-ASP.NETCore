...
@Component({
  ...
  styleUrls: ['./program-builder.component.less'],
  animations: [fadeInAnimation]
})
export class ProgramBuilderComponent implements OnInit, OnDestroy {
  ...
  public ViewDayMenu: boolean = false;
  ...
  drop(event: CdkDragDrop<ProgramDayItem[]>) {
    ...
    this.ProgramService.SetSavedProgram(this.Program);
    // //items were re-orderd
    // else { }
    // for (var i = 0; i < targetModel.length; i++)//always determin correct order
    // {
    //   (<ProgramDayItem>targetModel[i]).Position = i;
    // }
    // this.ProgramService.SetSavedProgram(this.Program);
  }
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(params => {
      if (params['id'] !== undefined) {
        ...
        this.ProgramService.ClearSavedProgram();
      }
      else {
        this.Program = this.ProgramService.GetSavedProgram();
  ...
  // RemoveItem(day: Day, item: ProgramDayItem) {
  //   var index = day.Items.findIndex(x => x.Id == item.Id);
  ...
  ToggleSuperSetSelectedWorkoutAdvancedOptions(s: SuperSet_Exercise) {
    s.SelectedWorkout.ShowAdvancedOptions = !s.SelectedWorkout.ShowAdvancedOptions;
  }
  ToggleSuperSetSelectedWorkoutPercentageBox(s: SuperSet_Exercise) {
    s.SelectedWorkout.ShowPercentageBox = !s.SelectedWorkout.ShowPercentageBox;
    if (!s.SelectedWorkout.ShowPercentageBox) {
      this.BlankOutSuperSetsAndRepsOptions(s, function (targetSet: Set) {
        targetSet.Percent = null;
      });
    }
  }
  ToggleSuperSetSelectedWorkoutSetsBox(s: SuperSet_Exercise) {
    s.SelectedWorkout.ShowSetsBox = !s.SelectedWorkout.ShowSetsBox;
    if (!s.SelectedWorkout.ShowSetsBox) {
      this.BlankOutSuperSetsAndRepsOptions(s, function (targetSet: Set) {
        targetSet.Sets = null;
      });
    }
  }
  ToggleSuperSetSelectedWorkoutRepsBox(s: SuperSet_Exercise) {
    s.SelectedWorkout.ShowRepsBox = !s.SelectedWorkout.ShowRepsBox;
    if (!s.SelectedWorkout.ShowRepsBox) {
      this.BlankOutSuperSetsAndRepsOptions(s, function (targetSet: Set) {
        targetSet.Reps = null;
      });
    }
  }
  ToggleSuperSetSelectedWorkoutWeightBox(s: SuperSet_Exercise) {
    s.SelectedWorkout.ShowWeight = !s.SelectedWorkout.ShowWeight;
    if (!s.SelectedWorkout.ShowWeight) {
      this.BlankOutSuperSetsAndRepsOptions(s, function (targetSet: Set) {
        targetSet.Weight = null;
      });
    }
  }
  ToggleSuperSetSelectedWorkoutTimeBox(s: SuperSet_Exercise) {
    s.SelectedWorkout.ShowTimeBox = !s.SelectedWorkout.ShowTimeBox;
    if (!s.SelectedWorkout.ShowTimeBox) {
      this.BlankOutSuperSetsAndRepsOptions(s, function (targetSet: Set) {
  ...
  ToggleSuperSetSelectedWorkoutRestBox(s: SuperSet_Exercise) {
    s.SelectedWorkout.ShowRestBox = !s.SelectedWorkout.ShowRestBox;
    if (!s.SelectedWorkout.ShowRestBox) {
      s.SelectedWorkout.Rest = null;
    }
  }
  ToggleSuperSetSelectedWorkoutShowOtherBox(s: SuperSet_Exercise) {
    s.SelectedWorkout.ShowOtherBox = !s.SelectedWorkout.ShowOtherBox;
    if (!s.SelectedWorkout.ShowOtherBox) {
      this.BlankOutSuperSetsAndRepsOptions(s, function (targetSet: Set) {
  ...
  ToggleSuperSetSelectedWorkoutDistanceBox(s: SuperSet_Exercise) {
    s.SelectedWorkout.ShowDistanceBox = !s.SelectedWorkout.ShowDistanceBox;
    if (!s.SelectedWorkout.ShowDistanceBox) {
      this.BlankOutSuperSetsAndRepsOptions(s, function (targetSet: Set) {
        targetSet.Distance = null;
      });
    }
  }
  ToggleSuperSetSelectedWorkoutRepsAchieved(s: SuperSet_Exercise,) {
    s.SelectedWorkout.ShowRepsAchievedBox = !s.SelectedWorkout.ShowRepsAchievedBox;
    this.BlankOutSuperSetsAndRepsOptions(s, function (targetSet: Set) {
      targetSet.RepsAchieved = s.SelectedWorkout.ShowRepsAchievedBox;
    });
  }
  ToggleDayMenu() {
    this.ViewDayMenu = !this.ViewDayMenu;
  }

  BlankOutSuperSetsAndRepsOptions(s: SuperSet_Exercise, clearOutFunction) {
    s.Weeks.forEach(x => {
  ...
  ToggleSelectedWorkoutAdvancedOptions(s: WorkoutDetails) {
